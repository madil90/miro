...
void Keyboard(unsigned char key, int x, int y)
{
    ...
    case 'P':
        {
            ...
            bool bRender = g_pScene->getPhotonManager()->getRenderMaps();
            g_pScene->getPhotonManager()->setRenderMaps(!bRender);
            break;
        }
    ...
    case 'C':
        {
            Vector3 g_vUp = g_pCamera->GetUp();
            Vector3 g_vViewDir = g_pCamera->GetViewDir();
            Vector3 g_vEye = g_pCamera->GetEye();

            Debug("Up vector is %f,%f,%f \n",g_vUp.x,g_vUp.y,g_vUp.z);
            Debug("View dir is %f,%f,%f \n",g_vViewDir.x,g_vViewDir.y,g_vViewDir.z);
            Debug("Eye pos is %f,%f,%f \n",g_vEye.x,g_vEye.y,g_vEye.z);
            break;
        }
        break;
