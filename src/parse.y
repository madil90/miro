...
%token PHOTONMAP
%token CAUSTICPHOTONS
%token INDIRECTPHOTONS
%token PHOTONSESTIMATE
%token CAUSTICESTIMATE
%token FINALGATHER
...
areaLightOptions: 
            | INTENSITY rExp areaLightOptions
            { 
            ...
            | SAMPLES rExp areaLightOptions 
                { pLight->SetSamples($2);} 
            | RENDERLIGHT rExp areaLightOptions
            {
                if ($2==0)
...
globalOptions: /* empty */
        ...
        | BSPSPLITS rExp globalOptions
            {BSPTree::setSplitChecks($2);}
        ...
        | CAUSTICPHOTONS rExp globalOptions
            { g_pScene->getPhotonManager()->setNoCausticPhotons($2);}
        | INDIRECTPHOTONS rExp globalOptions
            { g_pScene->getPhotonManager()->setNoIndirectPhotons($2);}
        | PHOTONSESTIMATE rExp globalOptions
            { g_pScene->getPhotonManager()->setNoPhotonsInEstimate($2);}
        | CAUSTICESTIMATE rExp globalOptions
            { g_pScene->getPhotonManager()->setNoPhotonsInCausticEstimate($2);}           
        | FINALGATHER rExp globalOptions
            { g_pScene->getPhotonManager()->setFinalGatherRays($2);}
;
