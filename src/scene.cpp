...
Scene::Scene () :
    m_bsp (new BSPTree ()) ,
        ...
        path_trace(false)
{
    // set photon mapping options
    debug_print = false;
    photon_map = false;
    pManager = new PhotonManager();
...
void Scene::PreCalc()
{
    ...
    // construct photon map if required
    if (photon_map)
        pManager->ShootPhotons(*this);
}

// get the refracted ray
bool Scene::getRefractedRay(HitInfo& hit, Ray& ray, Ray& refRay)
{
    float n1,n2;
...
Vector3 Scene::TraceRay(Ray& ray, Camera* cam, int depth)
{
    ...
    if (Trace(minHit,ray)){
        ...
        Vector3 diffColor = minHit.material->getDiffuse();
        ...
        // indirect illumination at diffuse surface
        if (!diffColor.isZero())
            if (ray.t != Ray::GATHER){
                ...
                // handle photon mapping
                if (photon_map){
                    color += pManager->radiance(minHit,*this);
                }
            }else{

                if (photon_map)
                    color += ArrayProd(diffColor/PI,
                            pManager->indirectIllum(minHit,*this));
            }
