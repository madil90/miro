...
Photon_map :: Photon_map( const int max_phot )
  : photons(1), max_power(0.0f)   // Ack! Henrik uses 1-indexed arrays.
//************************************************
{
  stored_photons = 0;
  prev_scale = 1;

  photons.reserve( max_phot+1 );
...
//******************************
{
  ...
  std::vector<Photon*> pa1(stored_photons+1);

  {
    std::vector<Photon*> pa2(stored_photons+1);
    for (int i=0; i<=stored_photons; i++)
    pa2[i] = &photons[i];

    balance_segment( &pa1[0], &pa2[0], 1, 1, stored_photons );
  }
